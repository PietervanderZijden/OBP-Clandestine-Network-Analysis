...
st.subheader("Remove a communication link (click)")

st.caption(
    "Removing some links can paradoxically improve global connectivity by eliminating bottlenecks."
)
...
if not sel:
    st.info("Click an edge (row) to remove it.")
    st.stop()
...
if nx.is_connected(G_removed):
    ...
    st.write(f"Largest component size: **{nodes.size}**")
